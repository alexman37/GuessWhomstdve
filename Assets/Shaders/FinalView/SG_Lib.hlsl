...
void SG_OverlayHair_float(float4 Base, float4 Top, out float4 Out) {
...
void SG_OverlayJob_float(float4 Base, float4 Top, out float4 Out) {
	if(distance(Top, C_HAIR_CUT) < epsilon) {
		...
		// Probably clothes or empty space
			...
			Out = Base.a > 0 ? Base : float4(0,0,0,0);
		}
	} else {
		Out = Top.a == 0 ? Base : Top;
	}
...
void SG_Colorize_float(float4 Uncolored, float4 SkinColor, float4 HairColor, float4 BodyColor, float4 EyeColor, out float4 Colored) {
	//Colored = Uncolored;
	if(Uncolored.a > epsilon) {
		if(Uncolored.r > epsilon) {
			SG_ColorSwap_float(Uncolored, C_SKIN_BASE, SkinColor, Colored);
			SG_ColorSwap_float(Colored, C_SKIN_OUTLINE, SkinColor * 0.8f, Colored);
		}

		SG_ColorSwap_float(Colored, C_EYE_BASE, EyeColor, Colored);

		if(Uncolored.g > epsilon) {
			SG_ColorSwap_float(Colored, C_BODY_BASE, BodyColor, Colored);
			SG_ColorSwap_float(Colored, C_BODY_OUTLINE, BodyColor * 0.8f, Colored);
		}

		if(Uncolored.b > epsilon) {
			SG_ColorSwap_float(Colored, C_HAIR_BASE, HairColor, Colored);
			SG_ColorSwap_float(Colored, C_HAIR_OUTLINE, HairColor * 0.8f, Colored);
			SG_ColorSwap_float(Colored, C_HAIR_SHADOW, HairColor * 0.6f, Colored);
			SG_ColorSwap_float(Colored, C_HAIR_BACK, HairColor * 0.4f, Colored);
		}
	}
}
